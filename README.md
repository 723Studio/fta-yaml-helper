...
By default, the extension treats each VS Code workspace folder as the OpenXcom mod root. If your ruleset is inside a larger repository, configure `oxcYamlHelper.modRoot` with the path from the workspace folder to the actual mod root.

For example, if your repository contains a mod under `content/From the Ashes`, use:
...
Standalone projects that should not inherit the bundled X-COM rulesets can also disable builtin base game loading:
