...
export class WorkspaceFolderRulesetHierarchy {
    ...
    public handleDeletes() {
        ...
        for (const idx in this.ruleset.rulesetFiles) {
            ...
            this.ruleset.rulesetFiles[idx].definitions = file.definitions.filter(
                (def) => !deletes.find((del) => del.key === def.name && del.path === def.type),
            );
    ...
    private getDefinitions() {
        ...
        const modFiles = this.ruleset.rulesetFiles.filter((file) =>
            file.file.path.startsWith(Uri.joinPath(hierarchy.vanilla, '/').path),
        );
